...
export default function SimpleChaewonPlant({ moisture, mood }: SimpleChaewonPlantProps) {
  ...
  const getChaewonImage = () => {
    switch (mood) {
      case 'love':
        ...
        return 'https://64.media.tumblr.com/8f1c4c8c8e8e8c8c8c8c8c8c8c8c8c8c/tumblr_inline_p7g8r8r8r8r8r8r-500.gif';
      case 'neutral':
        // Calm, peaceful Chaewon
        return 'https://kpop.fandom.com/wiki/Kim_Chaewon_(LE_SSERAFIM)/Gallery?file=IZONE_Kim_Chae_Won_official_profile_photo.png';
      case 'sad':
        // Sad, tired Chaewon
  ...
  return (
    <div className={`flex flex-col items-center justify-center min-h-screen bg-gradient-to-br ${colors.bg} p-4 relative overflow-hidden`}>
      ...
      <div className="relative mb-4 group mt-8">
        <div className={`w-60 h-60 rounded-full overflow-hidden border-6 ${colors.border} shadow-2xl ${colors.glow} transition-all duration-500 hover:scale-105`}>
          <img 
            src={getChaewonImage()} 
        ...
        {mood === 'love' && (
          <>
            ...
            <div className="absolute -bottom-2 -left-6 text-xl animate-pulse" style={{ animationDelay: '1s' }}>🌸</div>
            <div className="absolute top-2 -left-8 text-lg animate-spin" style={{ animationDelay: '2s', animationDuration: '3s' }}>💖</div>
            <div className="absolute top-4 -right-8 text-lg animate-spin" style={{ animationDelay: '1.5s', animationDuration: '4s' }}>⭐</div>
          </>
        )}
        {mood === 'neutral' && (
          <>
            <div className="absolute -top-3 -left-3 text-xl animate-pulse">🌸</div>
            <div className="absolute -top-1 -right-5 text-lg animate-pulse" style={{ animationDelay: '1s' }}>💜</div>
            <div className="absolute -bottom-3 -right-3 text-xl animate-pulse" style={{ animationDelay: '2s' }}>🌺</div>
            <div className="absolute -bottom-1 -left-5 text-lg animate-pulse" style={{ animationDelay: '1.5s' }}>✨</div>
          </>
        )}
        {mood === 'sad' && (
          <>
            <div className="absolute -top-3 -left-3 text-xl animate-bounce opacity-70">💧</div>
            <div className="absolute -top-1 -right-5 text-lg animate-pulse opacity-70" style={{ animationDelay: '1s' }}>😢</div>
            <div className="absolute -bottom-3 -right-3 text-xl animate-bounce opacity-70" style={{ animationDelay: '2s' }}>💙</div>
            <div className="absolute -bottom-1 -left-5 text-lg animate-pulse opacity-70" style={{ animationDelay: '1.5s' }}>🌧️</div>
            <div className="absolute top-1 -left-7 text-lg animate-pulse opacity-60" style={{ animationDelay: '3s' }}>💀</div>
            <div className="absolute top-3 -right-7 text-lg animate-bounce opacity-60" style={{ animationDelay: '2.5s' }}>⚰️</div>
          </>
        )}
      </div>
      ...
      <div className="w-full max-w-md mb-4">
        <div className="text-center mb-2">
          <span className={`text-lg font-bold ${colors.text}`}>💧 Emotional Water Level</span>
        </div>
        
        {/* Beautiful moisture bar with glow effect */}
        <div className="relative">
          <div className={`w-full h-6 bg-white/50 rounded-full overflow-hidden border-2 ${colors.border} backdrop-blur-sm shadow-lg`}>
            <div 
              className={`h-full transition-all duration-1000 bg-gradient-to-r ${colors.gradient} relative overflow-hidden`}
              style={{ width: `${Math.max(0, Math.min(100, displayMoisture))}%` }}
            >
          ...
          <div className="flex justify-between items-center mt-2">
            <span className={`text-2xl font-bold ${colors.text}`}>{displayMoisture}%</span>
            <span className={`text-sm font-semibold ${colors.text} flex items-center gap-1`}>
              {mood === 'love' ? '✨ Overflowing Era' : mood === 'neutral' ? '🌸 Balanced Era' : '💔 Drought Era'}
            </span>
          </div>
          ...
          <div className="flex justify-between mt-1 text-xs opacity-60">
            ...
            <span>{mood === 'love' ? 'Glowing' : mood === 'neutral' ? 'Content' : 'Hoping'}</span>
            <span>{mood === 'love' ? 'Radiant' : 'Flourishing'}</span>
          </div>
        </div>
      </div>
      ...
      <div className="w-full max-w-md mb-4">
        <div className={`${colors.barBg} text-white px-4 py-2 rounded-t-lg shadow-lg`}>
          <span className="font-bold text-base flex items-center gap-2">
            🌸 Chaewon Plant's Heart:
            {mood === 'love' && <span className="animate-pulse">💖</span>}
            {mood === 'neutral' && <span className="animate-pulse">💜</span>}
            {mood === 'sad' && <span className="animate-pulse">💙</span>}
          </span>
        </div>
        
        <div className="bg-white/90 backdrop-blur-sm border-2 border-current rounded-b-lg p-4 min-h-[80px] flex flex-col justify-center shadow-xl" style={{ borderColor: colors.border.replace('border-', '') }}>
          <div className="text-base text-gray-800 mb-2 italic leading-relaxed">
            "{currentQuote}"
          </div>
          <div className={`text-xs font-semibold flex items-center justify-between ${colors.text}`}>
            <span>- {currentSong}</span>
            <span className="text-xs opacity-70">🎵 Current Mood</span>
          </div>
        </div>
      </div>
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
        {[...Array(sparkleCount)].map((_, i) => {
          ...
          const isSpecialEmoji = (mood === 'sad' && (emoji === '💀' || emoji === '⚰️' || emoji === '💔'));
          
          return (
            <div
              key={i}
              className={`absolute text-2xl animate-float ${isSpecialEmoji ? 'opacity-80' : 'opacity-60'} ${isSpecialEmoji ? 'text-3xl' : ''}`}
              style={{
                left: `${Math.random() * 100}%`,
                top: `${Math.random() * 100}%`,
                animationDelay: `${Math.random() * 5}s`,
                animationDuration: `${3 + Math.random() * 4}s`,
                transform: isSpecialEmoji ? 'scale(1.2)' : 'scale(1)'
              }}
            >
              {emoji}
            </div>
      ...
      <div className="absolute bottom-2 right-2 flex gap-1">
        {[...Array(5)].map((_, i) => (
          <span key={i} className={`text-lg ${i < plantLevel ? 'opacity-100' : 'opacity-30'}`}>
            {mood === 'love' ? '💖' : mood === 'neutral' ? '💜' : '💙'}
          </span>
        ))}
      </div>
